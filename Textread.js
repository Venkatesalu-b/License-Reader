...
app.get('/gettext',async(req,res)=>{
    try{
      const text = await pool.query("select * from ImageText");
      res.json(text.rows)
      console.log(text);
    }
    catch(error){
  ...
  app.post('/users/images',async(req,res)=>{
    try{
        const{images}=req.body;
        const insert2 =await pool.query( "insert into ImageText (images) VALUES($1)",
        [images])
        res.json(insert2.rows[0])
        // console.log(res.json(insert2.rows));
            const [result] = await client.textDetection(images);
            console.log(result.fullTextAnnotation.text,'text');
